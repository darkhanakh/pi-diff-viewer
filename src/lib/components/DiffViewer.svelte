<script lang="ts">
	...
	import { getAnnotations, addAnnotation, removeAnnotation } from "$lib/annotations.svelte";
	import { Badge } from "$lib/components/ui/badge/index.js";
	import { Button } from "$lib/components/ui/button/index.js";

	type Props = { file: DiffFile };
	...
	const fileAnnotations = $derived(store.list.filter((a) => a.file === file.name));
	...
	function renderAnnotationElement(annotation: { metadata: { id: string; comment: string } }) {
		const wrapper = document.createElement("div");
		wrapper.style.cssText = "margin:4px 0;padding:0 8px;font-family:system-ui,-apple-system,sans-serif;";
		const container = document.createElement("div");
		container.style.cssText = "background:oklch(0.178 0 0);border:1px solid oklch(0.32 0.04 250);border-radius:8px;padding:10px 12px;display:flex;align-items:flex-start;gap:8px;";
		container.innerHTML = `
		...
		removeBtn.addEventListener("click", () => removeAnnotation(annotation.metadata.id));
	...
	function createInstance() {
		...
		instance = new FileDiffComponent({
			...
			renderAnnotation: renderAnnotationElement,
		});
		fullRender();
	}
	...
	onDestroy(() => { observer?.disconnect(); instance?.cleanUp(); });
</script>
